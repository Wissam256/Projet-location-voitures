...
void ajoutervoiture(Voiture V[],int N){
    ...
    for(int i=0;i<N;i++){
        ...
        scanf("%f",&V[i].prix);
        printf("DisponibilitÃ© (1 pour disponible, 0 pour non disponible) : ");
...
void affichervoiture(Voiture V[],int N){
    ...
    printf("\nListe des voitures :\n");
    printf(" Marque | ModÃ¨le | Prix/Jour | DisponibilitÃ©\n");
    printf("-------------------------------------------------------------------------------\n");
    for (int i= 0 ; i < N; i++){
            printf(" %s | %s | %.2f | %s \n",
                   V[i].marque,
...
void modifiervoiture(Voiture V[], int N) {
    ...
    printf("Entrez la marque ou le modÃ¨le Ã  modifier : ");
    scanf("%s", mod);
    ...
    for (int i = 0; i < N; i++) {
        if (strcmp(V[i].marque, mod) == 0 || strcmp(V[i].modele,mod) == 0) {
            ...
            printf("Nouveau prix : ");
            scanf("%f", &V[i].prix);
            printf("Nouvelle disponibilitÃ© (1: disponible, 0: non disponible) : ");
            scanf("%d", &V[i].disponible);
            printf("Modification rÃ©ussie.\n");
        }
    }
...
void supprimervoiture(Voiture V[], int N) {
    ...
    printf("Entrez la marque ou le modÃ¨le Ã  supprimer : ");
    scanf("%s", mod);
...
void triervoitures(Voiture V[], int N) {
    ...
    printf("\nVoitures triÃ©es par prix de location par jour.\n");
    for (int i = 0; i < N; i++) {
        printf(" %s | %s | %.2f | %s \n",
               V[i].marque,
               V[i].modele,
...
void enregistrercvs(Voiture V[],int N) {
    ...
    fprintf(f,"Marque, ModÃ¨le, Prix, DisponibilitÃ© \n");
